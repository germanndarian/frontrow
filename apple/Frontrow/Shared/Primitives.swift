...
/// Labels that wrap onto as many rows as they need — for listing what someone
/// follows without a horizontal scroll.
...
/// A left-to-right layout that wraps, which SwiftUI still doesn't ship.
struct FlowLayout: Layout {
    var spacing: CGFloat = 8

    func sizeThatFits(proposal: ProposedViewSize, subviews: Subviews, cache: inout ()) -> CGSize {
        let width = proposal.width ?? .infinity
        var rows: CGFloat = 1, x: CGFloat = 0, rowHeight: CGFloat = 0, height: CGFloat = 0
